...
class InceptionResNetV2ModelTrainer(BaseModelTrainer):
    def __init__(self, ffnn_params_parser: FFNNParamsParser, num_categories: str, training_comment: str) -> None:
        ...
        self.model_name = f"InceptionResNetV2_{data_augmentation_str}opt_{optimizer_str}_lr_{str(lr).replace('.', '')}_batch_size_{batch_size}_time_{datetime.now().strftime('%Y%m%d%H%M')}"
    
        print('Load model')
        model = tf.keras.applications.InceptionResNetV2(weights=None, input_shape=(224, 224, 3), classes=num_categories)
        model.summary()
